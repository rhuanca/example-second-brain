#!/usr/bin/env bash
#
# Deploy the Second Brain *Telegram* (capture) bot as a systemd user service.
# Idempotent — safe to re-run after pulling code changes.
#
# One of two services in this project, sharing the same config and vault:
#   rr-second-brain-telegram  -> Telegram (capture)   [this script]
...
#   4. Writes ~/.config/systemd/user/rr-second-brain-telegram.service
...
SERVICE_NAME="rr-second-brain-telegram"
